...
class BTLeafNode {
  public:
    ...
    */
    ...
    //Size of single entry in a leaf node
    static const int SIZE_ENTRY = sizeof(RecordId) + sizeof(int);
    static const int MAX_ENTRY_NUM = (PageFile::PAGE_SIZE - sizeof(PageId)) / SIZE_ENTRY;
    static void popBufferEntry(int index, char* buffer, RecordId& rid, int& key);
    static void pushBufferEntry(RecordId rid, int key, char* buffer);	
    void initialBuffer() ;
  private:
   /**
...
class BTNonLeafNode {
  public:
    ...
    static const int SIZE_ENTRY = sizeof(NonLeafEntry);
    static const int MAX_ENTRY_NUM = (PageFile::PAGE_SIZE - sizeof(PageId)) / SIZE_ENTRY;
    static void popBufferEntry(int index, char* buffer, NonLeafEntry& entry);
    static void pushBufferEntry( PageId pid, int key, char* buffer);
    RC locate(int searchKey, int& eid );
